...
Get-ChildItem -name -r | select-string -Pattern "src\\HexaEmployee.*\\bin" | ForEach-Object { Remove-Item $_ -Recurse -Force -Confirm:$false | Out-Null }
Get-ChildItem -name -r | select-string -Pattern "src\\HexaEmployee.*\\obj" | ForEach-Object { Remove-Item $_ -Recurse -Force -Confirm:$false | Out-Null }
Get-ChildItem -name -r | select-string -Pattern "__tests__\\HexaEmployee.*\\bin" | ForEach-Object { Remove-Item $_ -Recurse -Force -Confirm:$false | Out-Null }
Get-ChildItem -name -r | select-string -Pattern "__tests__\\HexaEmployee.*\\obj" | ForEach-Object { Remove-Item $_ -Recurse -Force -Confirm:$false | Out-Null }
